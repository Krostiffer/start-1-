...
strlena a i = if' (i < 0) 0 (if' (i == 0) (lena a i) ((lena a i) + (strlena a (i-1)))) --checks for edge-case empty list [], else recursively adds the length of all substrings beginning with the last; returns length of concatted a 
strlenb :: CSMInstance -> Int -> Int
strlenb a i = if' (i < 0) 0 (if' (i == 0) (lenb a i) ((lenb a i) + (strlenb a (i-1)))) --checks for edge-case empty list [], else recursively adds the length of all substrings beginning with the last; returns length of concatted b